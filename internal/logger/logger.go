...
var (
	...
	eventLogger   *WindowsEventLogger
...
func init() {
	...
	loggingConfig = LoggingConfig{
		UseEventLog: true,  // Enable Windows Event Log by default on Windows
		UseFileLog:  true,  // Always enable file logging
		LogLevel:    logLevel,
	}
...
func (l *Logger) log(level int, format string, args ...interface{}) {
	...
	// Write to multiple outputs based on configuration
	l.writeToOutputs(level, message, logLine)
...
// initializeEnhancedLogging initializes Windows Event Log and File Logger
func initializeEnhancedLogging() {
	// Initialize Windows Event Logger (only on Windows)
	if loggingConfig.UseEventLog {
		if elog, err := NewWindowsEventLogger(); err == nil {
			eventLogger = elog
		}
	}

	// Initialize File Logger
	if loggingConfig.UseFileLog {
		if flog, err := NewFileLogger(); err == nil {
			fileLogger = flog
		}
	}
}

// writeToOutputs writes log message to appropriate outputs based on level and configuration
func (l *Logger) writeToOutputs(level int, message, logLine string) {
	// Always write DEBUG messages to file only
	if level == LevelDebug {
		if fileLogger != nil {
			if file, err := fileLogger.OpenLogFile(); err == nil {
	...
	// For INFO, WARNING, ERROR, FATAL: write to both Event Log and File
	
	// Write to Windows Event Log (INFO and above)
	if eventLogger != nil && level >= LevelInfo {
		switch level {
		case LevelInfo:
	...
	// Also write to standard output (backwards compatibility)
	l.out.Write([]byte(logLine))
}

// ConfigureLogging allows runtime configuration of logging options
func ConfigureLogging(config LoggingConfig) {
	loggingConfig = config
	logLevel = config.LogLevel
	SetLevel(config.LogLevel)
	
	// Reinitialize loggers with new config
	initializeEnhancedLogging()
}
...
// CleanupLogging closes all logging resources
func CleanupLogging() {
	if eventLogger != nil {
		eventLogger.Close()
	}
