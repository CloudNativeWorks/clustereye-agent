...
func (p *PostgreSQLMetricsCollector) CollectSystemMetrics() (*MetricBatch, error) {
	...
	if filesystem, ok := systemMetrics["filesystem"].(string); ok && filesystem != "N/A" {
		metrics = append(metrics, Metric{
			Name:        "postgresql.system.filesystem",
	...
	if mountPoint, ok := systemMetrics["mount_point"].(string); ok && mountPoint != "N/A" {
		metrics = append(metrics, Metric{
			Name:        "postgresql.system.mount_point",
	...
	if usagePercent, ok := systemMetrics["usage_percent"].(int); ok {
		...
		metrics = append(metrics, Metric{
			Name:        "postgresql.system.usage_percent",
...
func (p *PostgreSQLMetricsCollector) CollectDatabaseMetrics() (*MetricBatch, error) {
	...
	// Collect database sizes for capacity planning
	p.collectDatabaseSizes(db, &metrics, timestamp)
...
// collectDatabaseSizes collects database size metrics for capacity planning
func (p *PostgreSQLMetricsCollector) collectDatabaseSizes(db *sql.DB, metrics *[]Metric, timestamp int64) {
	query := `
		SELECT 
			datname,
	...
	for rows.Next() {
		var datname, sizeFormatted string
		var sizeBytes int64

		if err := rows.Scan(&datname, &sizeBytes, &sizeFormatted); err != nil {
			logger.Error("Failed to scan database size row: %v", err)
			continue
		}
		...
		// Database size in bytes (for calculations)
		*metrics = append(*metrics, Metric{
			Name:        "postgresql.database.size_bytes",
			Value:       MetricValue{IntValue: &sizeBytes},
			Tags:        tags,
			Timestamp:   timestamp,
			Unit:        "bytes",
		...
		// Database size in MB (for easier reading)
		sizeMB := float64(sizeBytes) / (1024 * 1024)
		*metrics = append(*metrics, Metric{
			Name:        "postgresql.database.size_mb",
			Value:       MetricValue{DoubleValue: &sizeMB},
			Tags:        tags,
			Timestamp:   timestamp,
			Unit:        "megabytes",
