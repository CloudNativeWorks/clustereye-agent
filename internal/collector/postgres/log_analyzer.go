...
var (
	...
	// Fixed regex to properly capture log level after client field
	postgresUTCRegex = regexp.MustCompile(`^([\d-]+ [\d:.]+(?:\.\d+)?)\s+UTC\s+\[(\d+)\]\s+(?:user=([^,]*),db=([^,]*),app=[^,]*,client=[^\s]*\s+)?(\w+):\s+(.*)$`)
	...
	// - 697.695 ms  execute <unnamed>: SELECT ... (PostgreSQL format)
	durationRegex = regexp.MustCompile(`(?:duration:\s*([\d.]+)\s*ms|^([\d.]+)\s*ms\s+)`)
...
func AnalyzePostgresLog(logFilePath string, slowQueryThresholdMs int64) (*pb.PostgresLogAnalyzeResponse, error) {
	...
	for _, entry := range logEntries {
		...
		// 1. SQL sorguları (InternalQuery dolu)
		// 2. Duration bilgisi olan loglar
		// 3. ERROR, FATAL, WARNING log level'ına sahip loglar (LogLevel field'ından kontrol et)
		// 4. Message içinde ERROR/FATAL/WARNING olan loglar (eski format için)
		// 5. Önemli LOG mesajları (sistem logları hariç)
	...
	// ERROR'ları ayrı olarak topla - bunlar her zaman öncelikli
	var errorEntries []*pb.PostgresLogEntry
	for _, entry := range sqlEntries {
		if entry.LogLevel == "ERROR" || entry.LogLevel == "FATAL" || entry.LogLevel == "PANIC" {
			errorEntries = append(errorEntries, entry)
		}
	}

	// Sonuç belirleme mantığı - ERROR'lar her zaman dahil edilsin
	if slowQueryThresholdMs > 0 && len(slowQueryEntries) > 0 {
		// Yavaş sorgular + ERROR'lar
		resultEntries = slowQueryEntries
		resultEntries = append(resultEntries, errorEntries...)
		log.Printf("Yavaş sorgu eşiğini (%d ms) geçen %d adet + %d adet ERROR girdi döndürülüyor",
			slowQueryThresholdMs, len(slowQueryEntries), len(errorEntries))
	} else if slowQueryThresholdMs > 0 && len(slowQueryEntries) == 0 {
		// Yavaş sorgu yoksa, SQL girdileri + ERROR'lar (ERROR'lar zaten SQL entries'in içinde)
		if len(sqlEntries) > 0 {
			// En fazla 100 SQL girdi döndür (ERROR'lar dahil)
			maxSample := 100
			...
			log.Printf("Yavaş sorgu bulunamadı, %d adet SQL log kaydı döndürülüyor (ERROR'lar dahil)", len(resultEntries))
...
func parseLogLine(line string, matches []string, buffer *LogBuffer) bool {
	...
	if strings.Contains(message, "statement:") || strings.Contains(message, "STATEMENT:") {
		...
		// Handle both formats:
		// - "duration: 634.738 ms  execute <unnamed>: SELECT ..."
		// - "697.695 ms  execute <unnamed>: SELECT ..."

		// First, extract the SQL query part
	...
	if durMatch := durationRegex.FindStringSubmatch(message); durMatch != nil {
		...
		if durMatch[1] != "" {
			durationStr = durMatch[1] // duration: format
		} else if durMatch[2] != "" {
			durationStr = durMatch[2] // PostgreSQL format (number at start)
		}

		if durationStr != "" {
			duration, err := strconv.ParseFloat(durationStr, 64)
			if err == nil {
				// Convert to microseconds to preserve decimal precision, then back to milliseconds
				// 713.431 ms -> 713431 microseconds -> store as 713431 (then API can divide by 1000)
				buffer.duration = int64(duration * 1000)
