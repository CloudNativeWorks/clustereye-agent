...
func (c *MSSQLCollector) isNodeDefinitelyStandalone() bool {
	...
	var hasAvailabilityGroups int
	var hasAvailabilityReplicas int
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	err = db.QueryRowContext(ctx, `
		SELECT 
			CASE WHEN SERVERPROPERTY('IsHadrEnabled') = 1 THEN 1 ELSE 0 END AS IsHadrEnabled,
			CASE WHEN EXISTS (SELECT 1 FROM sys.availability_groups) THEN 1 ELSE 0 END AS HasGroups,
			CASE WHEN EXISTS (SELECT 1 FROM sys.availability_replicas) THEN 1 ELSE 0 END AS HasReplicas
	`).Scan(&alwaysOnConfigured, &hasAvailabilityGroups, &hasAvailabilityReplicas)
	...
	log.Printf("AlwaysOn kontrol sonuçları: IsHadrEnabled=%d, HasGroups=%d, HasReplicas=%d",
		alwaysOnConfigured, hasAvailabilityGroups, hasAvailabilityReplicas)

	if alwaysOnConfigured == 1 || hasAvailabilityGroups == 1 || hasAvailabilityReplicas == 1 {
		log.Printf("Bu instance'da AlwaysOn yapılandırılmış (HADR=%d, Groups=%d, Replicas=%d), standalone değil",
			alwaysOnConfigured, hasAvailabilityGroups, hasAvailabilityReplicas)
		return false
	}
...
func (c *MSSQLCollector) BatchCollectSystemMetrics() map[string]interface{} {
	log.Printf("[MSSQL COLLECTOR DEBUG] BatchCollectSystemMetrics fonksiyonu çağrıldı, OS: %s", runtime.GOOS)
	...
	log.Printf("[MSSQL COLLECTOR DEBUG] CPU kullanımı ölçümü başlıyor...")

	// 1. Yöntem: WMIC ile CPU kullanımını al
	log.Printf("[MSSQL COLLECTOR DEBUG] 1. Yöntem: WMIC komutu deneniyor...")
	...
	if err == nil {
		outStr := strings.TrimSpace(string(out))
		log.Printf("[MSSQL COLLECTOR DEBUG] WMIC çıktısı: %q", outStr)

		if matches := regexp.MustCompile(`LoadPercentage=(\d+)`).FindStringSubmatch(outStr); len(matches) > 1 {
			log.Printf("[MSSQL COLLECTOR DEBUG] Regex eşleşmeleri: %v", matches)
			if usage, err := strconv.ParseFloat(matches[1], 64); err == nil {
				metrics["cpu_usage"] = usage
				cpuUsageFound = true
				log.Printf("[MSSQL COLLECTOR DEBUG] ✓ WMIC ile CPU kullanımı başarıyla alındı: %.2f%%", usage)
			} else {
				log.Printf("[MSSQL COLLECTOR DEBUG] ✗ WMIC parse hatası: %v", err)
			}
		} else {
			log.Printf("[MSSQL COLLECTOR DEBUG] ✗ WMIC regex eşleşmedi")
		}
	} else {
		log.Printf("[MSSQL COLLECTOR DEBUG] ✗ WMIC komutu hatası: %v", err)
	}

	// 2. Yöntem: WMIC başarısız olursa typeperf kullan (Windows 2016 için daha güvenilir)
	if !cpuUsageFound {
		log.Printf("[MSSQL COLLECTOR DEBUG] 2. Yöntem: TypePerf komutu deneniyor...")
		...
		if err == nil {
			outStr := strings.TrimSpace(string(out))
			log.Printf("[MSSQL COLLECTOR DEBUG] TypePerf çıktısı: %q", outStr)

			lines := strings.Split(outStr, "\n")
			log.Printf("[MSSQL COLLECTOR DEBUG] TypePerf satır sayısı: %d", len(lines))

			// typeperf çıktısı CSV formatında, genellikle 3. satırda değer var
			for i, line := range lines {
				log.Printf("[MSSQL COLLECTOR DEBUG] TypePerf satır %d: %q", i, line)
				if strings.Contains(line, ",") && !strings.Contains(line, "PDH") {
					fields := strings.Split(line, ",")
					log.Printf("[MSSQL COLLECTOR DEBUG] TypePerf alanları: %v", fields)

					if len(fields) >= 2 {
						// Tüm gereksiz karakterleri agresif bir şekilde temizle
						valueStr := fields[1]
						// Tüm tırnak işaretlerini kaldır
						valueStr = strings.ReplaceAll(valueStr, "\"", "")
						// Tüm whitespace karakterlerini kaldır
						valueStr = strings.TrimSpace(valueStr)
						valueStr = strings.ReplaceAll(valueStr, "\r", "")
						valueStr = strings.ReplaceAll(valueStr, "\n", "")
						valueStr = strings.ReplaceAll(valueStr, "\t", "")
						log.Printf("[MSSQL COLLECTOR DEBUG] TypePerf temizlenmiş değer: %q", valueStr)

						// Sadece sayısal değerleri parse etmeye çalış
						if valueStr != "" && !strings.Contains(valueStr, "wait") && !strings.Contains(valueStr, "PDH") {
							if cpuVal, err := strconv.ParseFloat(valueStr, 64); err == nil {
								metrics["cpu_usage"] = cpuVal
								cpuUsageFound = true
								log.Printf("[MSSQL COLLECTOR DEBUG] ✓ TypePerf ile CPU kullanımı başarıyla alındı: %.2f%%", cpuVal)
								break
							} else {
								log.Printf("[MSSQL COLLECTOR DEBUG] ✗ TypePerf parse hatası: %v", err)
							}
						} else {
							log.Printf("[MSSQL COLLECTOR DEBUG] TypePerf değeri sayısal değil, atlanıyor: %q", valueStr)
						}
					}
				}
			}
			...
			if !cpuUsageFound {
				log.Printf("[MSSQL COLLECTOR DEBUG] ✗ TypePerf'dan uygun değer bulunamadı")
			}
		} else {
			log.Printf("[MSSQL COLLECTOR DEBUG] ✗ TypePerf komutu hatası: %v", err)
		}
	}

	// 3. Yöntem: PowerShell ile CPU kullanımını al (son çare)
	if !cpuUsageFound {
		log.Printf("[MSSQL COLLECTOR DEBUG] 3. Yöntem: PowerShell komutu deneniyor...")
		...
		if err == nil {
			...
			log.Printf("[MSSQL COLLECTOR DEBUG] PowerShell çıktısı: %q", cpuStr)

			if cpuVal, err := strconv.ParseFloat(cpuStr, 64); err == nil {
				metrics["cpu_usage"] = cpuVal
				cpuUsageFound = true
				log.Printf("[MSSQL COLLECTOR DEBUG] ✓ PowerShell ile CPU kullanımı başarıyla alındı: %.2f%%", cpuVal)
			} else {
				log.Printf("[MSSQL COLLECTOR DEBUG] ✗ PowerShell parse hatası: %v", err)
			}
		} else {
			log.Printf("[MSSQL COLLECTOR DEBUG] ✗ PowerShell komutu hatası: %v", err)
		}
	}

	if !cpuUsageFound {
		log.Printf("[MSSQL COLLECTOR DEBUG] ✗ Tüm yöntemler başarısız, CPU kullanımı alınamadı")
	}
	...
	log.Printf("[MSSQL COLLECTOR DEBUG] Final metrics: cpu_usage=%.2f, cpu_count=%v, memory_usage=%.2f",
		getFloatValue(metrics, "cpu_usage"), metrics["cpu_count"], getFloatValue(metrics, "memory_usage"))

	return metrics
}

// Helper function to safely get float value from metrics map
...
func (c *MSSQLCollector) GetMSSQLInfo() *MSSQLInfo {
	...
	// SON KRİTİK KONTROL: Çelişkili durumları tespit et ve düzelt
	if info.NodeStatus == "STANDALONE" && (info.ClusterName != "" || info.IsHAEnabled) {
		log.Printf("KRİTİK UYARI: Çelişkili durum tespit edildi - NodeStatus=STANDALONE ama ClusterName='%s' veya IsHAEnabled=%v",
			info.ClusterName, info.IsHAEnabled)
		log.Printf("Bu failover anında yakalanmış olabilir, bilgiler düzeltiliyor")

		// Çelişkili durumda önbelleğe güven ve UNKNOWN olarak işaretle
		info.NodeStatus = "UNKNOWN"
		info.HARole = "UNKNOWN"

		// Eğer ClusterName varsa, kesinlikle HA enabled olmalı
		if info.ClusterName != "" {
			info.IsHAEnabled = true
		}

		log.Printf("Düzeltilmiş durumlar: NodeStatus=%s, HARole=%s, IsHAEnabled=%v",
			info.NodeStatus, info.HARole, info.IsHAEnabled)
	}

	// Eğer NodeStatus UNKNOWN ise ve ClusterName varsa, bu da potansiyel bir failover durumudur
	if info.NodeStatus == "UNKNOWN" && info.ClusterName != "" {
		log.Printf("UYARI: NodeStatus=UNKNOWN ama ClusterName='%s' - failover durumunda olabilir", info.ClusterName)
		info.IsHAEnabled = true // Kesinlikle HA olarak işaretle
		log.Printf("IsHAEnabled true olarak ayarlandı")
	}

	logger.Info("MSSQL bilgileri başarıyla toplandı. IP=%s, Status=%s, HAEnabled=%v, NodeStatus=%s",
		info.IP, info.Status, info.IsHAEnabled, info.NodeStatus)
...
func (m *MSSQLInfo) ToProto() *pb.MSSQLInfo {
	// PROTOBUF DÖNÜŞTÜRMEDEKİ SON KRİTİK KONTROL: Çelişkili durumları tespit et
	if m.NodeStatus == "STANDALONE" && (m.ClusterName != "" || m.IsHAEnabled) {
		log.Printf("PROTOBUF ÖNEMLİ UYARI: Çelişkili durum tespit edildi - NodeStatus=STANDALONE ama ClusterName='%s' veya IsHAEnabled=%v",
			m.ClusterName, m.IsHAEnabled)
		...
		log.Printf("PROTOBUF düzeltilmiş durumlar: NodeStatus=%s, HARole=%s, IsHAEnabled=%v",
			m.NodeStatus, m.HARole, m.IsHAEnabled)
	}
