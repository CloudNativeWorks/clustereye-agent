...
// GetDiskUsage disk kullanım bilgilerini döndürür: (totalSize, freeSize, usagePercent, filesystem, mountPoint)
func (c *MongoCollector) GetDiskUsage() (string, string, int, string, string) {
	...
	for _, line := range lines[1:] {
		...
		// MongoDB için özel mount point kontrol - /mongodata varsa onu seç
		if strings.Contains(mountPoint, "mongo") || mountPoint == "/mongodata" {
			maxSize = sizeInBytes
			selectedTotal = size
			selectedFree = free
			...
			logger.Debug("MongoDB GetDiskUsage - MONGO-SPECIFIC SELECTED: %s -> %s (Size: %s, Free: %s, Usage: %s%%)",
				selectedFilesystem, selectedMountPoint, selectedTotal, selectedFree, usage)
		} else if sizeInBytes > maxSize || mountPoint == "/" {
	...
	return selectedTotal, selectedFree, selectedUsage, selectedFilesystem, selectedMountPoint
}
...
func (c *MongoCollector) convertToBytes(size string) (uint64, error) {
	logger.Debug("MongoDB convertToBytes - Input: '%s'", size)

	size = strings.TrimSpace(size)
	if len(size) == 0 {
		logger.Debug("MongoDB convertToBytes - Empty size input")
	...
	logger.Debug("MongoDB convertToBytes - Parsed: num=%f, unit=%s", num, unit)
	...
	logger.Debug("MongoDB convertToBytes - Output: %d bytes (num=%f, unit=%s, multiplier=%d)", result, num, unit, multiplier)
...
func (c *MongoCollector) GetMongoInfo() *MongoInfo {
	...
	ip := c.getLocalIP()
	_, freeDisk, usagePercent, _, _ := c.GetDiskUsage()
...
// GetCurrentOp MongoDB'den aktif operasyonları getirir
func (c *MongoCollector) GetCurrentOp(secsRunning float64) (map[string]interface{}, error) {
	// Panic recovery
	defer func() {
		if r := recover(); r != nil {
			logger.Error("PANIC in GetCurrentOp: %v", r)
			buf := make([]byte, 4096)
			n := runtime.Stack(buf, false)
			logger.Error("MongoDB GetCurrentOp STACK TRACE: %s", string(buf[:n]))
		}
	}()

	log.Printf("[DEBUG] MongoDB GetCurrentOp çağrıldı, secsRunning=%.1f", secsRunning)

	// MongoDB client'ı al
	client, err := c.GetClient()
	if err != nil {
		log.Printf("[ERROR] MongoDB client alınamadı: %v", err)
		return nil, fmt.Errorf("MongoDB client alınamadı: %v", err)
	}
	defer client.Disconnect(context.Background())

	// Admin database'ine bağlan
	adminDB := client.Database("admin")

	// currentOp komutunu oluştur - sadece aktif operasyonları al, sonra Go tarafında filtrele
	currentOpFilter := bson.M{
		"active": true,
	}

	log.Printf("[DEBUG] MongoDB currentOp filtresi: %+v", currentOpFilter)
	...
	err = adminDB.RunCommand(ctx, bson.M{
		"currentOp": currentOpFilter,
	}).Decode(&result)

	if err != nil {
		log.Printf("[ERROR] MongoDB currentOp komutu başarısız: %v", err)
		return nil, fmt.Errorf("currentOp komutu başarısız: %v", err)
	}

	log.Printf("[INFO] MongoDB currentOp komutu başarıyla çalıştırıldı")

	// BSON'u map[string]interface{} formatına çevir
	resultMap := make(map[string]interface{})
	bsonBytes, err := bson.Marshal(result)
	if err != nil {
		log.Printf("[ERROR] BSON marshal hatası: %v", err)
		return nil, fmt.Errorf("BSON marshal hatası: %v", err)
	}

	err = bson.Unmarshal(bsonBytes, &resultMap)
	if err != nil {
		log.Printf("[ERROR] BSON unmarshal hatası: %v", err)
		return nil, fmt.Errorf("BSON unmarshal hatası: %v", err)
	}

	// MongoDB'den dönen sonucu detaylı logla
	log.Printf("[DEBUG] MongoDB currentOp sonucu alanları:")
	for key, value := range resultMap {
		log.Printf("[DEBUG]   %s: %v (tip: %T)", key, value, value)
	}

	// Go tarafında filtreleme yap - farklı tipleri dene
	var inprog []interface{}
	var foundInprog bool

	// Farklı tipleri dene
	if inprogSlice, ok := resultMap["inprog"].([]interface{}); ok {
		inprog = inprogSlice
		foundInprog = true
		log.Printf("[DEBUG] inprog []interface{} tipinde bulundu")
	} else if inprogSlice, ok := resultMap["inprog"].([]map[string]interface{}); ok {
		// []map[string]interface{} tipini []interface{} tipine çevir
		inprog = make([]interface{}, len(inprogSlice))
		for i, item := range inprogSlice {
			inprog[i] = item
		}
		foundInprog = true
		log.Printf("[DEBUG] inprog []map[string]interface{} tipinde bulundu ve çevrildi")
	} else if inprogPrimitiveA, ok := resultMap["inprog"].(primitive.A); ok {
		// primitive.A tipini []interface{} tipine çevir
		inprog = make([]interface{}, len(inprogPrimitiveA))
		for i, item := range inprogPrimitiveA {
			inprog[i] = item
		}
		foundInprog = true
		log.Printf("[DEBUG] inprog primitive.A tipinde bulundu ve çevrildi, %d operasyon", len(inprog))
	} else {
		log.Printf("[DEBUG] inprog alanı desteklenen tipte değil: %T", resultMap["inprog"])
	}

	if foundInprog {
		log.Printf("[DEBUG] Filtreleme öncesi operasyon sayısı: %d", len(inprog))

		filteredOps := make([]interface{}, 0)

		for _, op := range inprog {
			if opMap, ok := op.(map[string]interface{}); ok {
				// secs_running kontrolü - daha detaylı debugging
				secsRunningVal, hasSecsRunning := opMap["secs_running"]
				log.Printf("[DEBUG] Operasyon analizi: hasSecsRunning=%v, secsRunningVal=%v (tip: %T)", hasSecsRunning, secsRunningVal, secsRunningVal)

				if hasSecsRunning {
					var actualSecsRunning float64
					switch v := secsRunningVal.(type) {
					case float64:
						actualSecsRunning = v
					case int:
						actualSecsRunning = float64(v)
					case int32:
						actualSecsRunning = float64(v)
					case int64:
						actualSecsRunning = float64(v)
					case float32:
						actualSecsRunning = float64(v)
					default:
						log.Printf("[DEBUG] Bilinmeyen secs_running tipi: %T, değer: %v", v, v)
						actualSecsRunning = 0
					}

					log.Printf("[DEBUG] secs_running karşılaştırması: %.3f < %.1f = %v", actualSecsRunning, secsRunning, actualSecsRunning < secsRunning)

					// Belirtilen süreden kısa çalışanları filtrele
					if actualSecsRunning < secsRunning {
						log.Printf("[DEBUG] Operasyon filtrelendi: secs_running=%.3f < %.1f", actualSecsRunning, secsRunning)
						continue
					}
				} else {
					// secs_running alanı yoksa, operasyonun tüm alanlarını logla
					log.Printf("[DEBUG] secs_running alanı yok, operasyon alanları:")
					for key, value := range opMap {
						log.Printf("[DEBUG]   %s: %v (tip: %T)", key, value, value)
					}
				}

				// Namespace kontrolü - admin komutlarını filtrele
				if ns, hasNs := opMap["ns"].(string); hasNs {
					if strings.HasPrefix(ns, "admin.") {
						log.Printf("[DEBUG] Admin namespace filtrelendi: %s", ns)
						continue
					}
				}

				// Anlamsız operasyonları filtrele
				// op: none olan operasyonları filtrele
				if op, hasOp := opMap["op"].(string); hasOp && op == "none" {
					log.Printf("[DEBUG] op:none operasyonu filtrelendi")
					continue
				}

				// Sadece anlamlı operasyon tiplerini geçir (daha geniş liste)
				if op, hasOp := opMap["op"].(string); hasOp {
					meaningfulOps := []string{"query", "insert", "update", "remove", "getmore", "command", "aggregate", "find", "delete", "count", "distinct", "findAndModify", "mapReduce"}
					ismeaningful := false
					for _, meaningfulOp := range meaningfulOps {
						if op == meaningfulOp {
							ismeaningful = true
							break
						}
					...
					if !ismeaningful {
						log.Printf("[DEBUG] Anlamsız operasyon tipi filtrelendi: %s", op)
						continue
					}
					log.Printf("[DEBUG] Anlamlı operasyon tipi geçti: %s", op)
				}

				// Sadece command operasyonları için boş command kontrolü yap
				if op, hasOp := opMap["op"].(string); hasOp && op == "command" {
					if command, hasCommand := opMap["command"].(map[string]interface{}); hasCommand {
						if len(command) == 0 {
							log.Printf("[DEBUG] Boş command operasyonu filtrelendi")
							continue
						}
					} else {
						log.Printf("[DEBUG] command alanı olmayan command operasyonu filtrelendi")
						continue
					}
				}

				// JournalFlusher gibi sistem operasyonlarını filtrele
				if desc, hasDesc := opMap["desc"].(string); hasDesc {
					if strings.Contains(desc, "JournalFlusher") ||
						strings.Contains(desc, "LogicalSessionCacheRefresh") ||
						strings.Contains(desc, "PeriodicTask") {
						log.Printf("[DEBUG] Sistem operasyonu filtrelendi: %s", desc)
						continue
					}
				}

				// Command kontrolü - hello, isMaster komutlarını filtrele
				// Önce doğrudan operasyon seviyesinde kontrol et
				if _, hasHello := opMap["hello"]; hasHello {
					log.Printf("[DEBUG] Hello komutu filtrelendi (operasyon seviyesinde)")
					continue
				}
				if _, hasHelloOk := opMap["helloOk"]; hasHelloOk {
					log.Printf("[DEBUG] HelloOk komutu filtrelendi")
					continue
				}
				if db, hasDb := opMap["$db"].(string); hasDb && db == "admin" {
					log.Printf("[DEBUG] Admin $db komutu filtrelendi")
					continue
				}

				// Sonra command içinde kontrol et
				if command, hasCommand := opMap["command"].(map[string]interface{}); hasCommand {
					if _, hasHello := command["hello"]; hasHello {
						log.Printf("[DEBUG] Hello komutu filtrelendi (command içinde)")
						continue
					}
					if _, hasIsMaster := command["isMaster"]; hasIsMaster {
						log.Printf("[DEBUG] isMaster komutu filtrelendi")
						continue
					}
					if _, hasIsmaster := command["ismaster"]; hasIsmaster {
						log.Printf("[DEBUG] ismaster komutu filtrelendi")
				...
				// Filtreyi geçen operasyonu ekle
				if op, hasOp := opMap["op"].(string); hasOp {
					log.Printf("[DEBUG] ✅ Operasyon filtreyi geçti: op=%s, secs_running=%v", op, opMap["secs_running"])
				} else {
					log.Printf("[DEBUG] ✅ Operasyon filtreyi geçti: op=unknown, secs_running=%v", opMap["secs_running"])
				}
				filteredOps = append(filteredOps, op)
			}
		}

		// Filtrelenmiş operasyonları geri koy
		resultMap["inprog"] = filteredOps
		log.Printf("[INFO] MongoDB currentOp sonucu: %d operasyon filtreleme öncesi, %d operasyon filtreleme sonrası", len(inprog), len(filteredOps))
	} else {
		log.Printf("[INFO] MongoDB currentOp sonucu alındı, inprog alanı işlenemedi")

		// Alternatif alanları kontrol et
		if ops, ok := resultMap["operations"].([]interface{}); ok {
			log.Printf("[DEBUG] 'operations' alanı bulundu, %d operasyon var", len(ops))
			resultMap["inprog"] = ops // operations'ı inprog olarak koy
		} else if cursor, ok := resultMap["cursor"].(map[string]interface{}); ok {
			if firstBatch, ok := cursor["firstBatch"].([]interface{}); ok {
				log.Printf("[DEBUG] 'cursor.firstBatch' alanı bulundu, %d operasyon var", len(firstBatch))
				resultMap["inprog"] = firstBatch // firstBatch'i inprog olarak koy
			}
		} else {
			log.Printf("[DEBUG] Hiçbir alternatif operasyon alanı bulunamadı")
		}
	}
...
// ListMongoDatabases MongoDB veritabanlarının listesini getirir
func (c *MongoCollector) ListMongoDatabases() ([]map[string]interface{}, error) {
	logger.Info("ListMongoDatabases fonksiyonu çağrıldı")
	
	// Bağlantı bilgilerini logla
	logger.Info("MongoDB bağlantı bilgileri - Host: %s, Port: %d, User: %s, Auth: %v", 
		c.cfg.Mongo.Host, c.cfg.Mongo.Port, c.cfg.Mongo.User, c.cfg.Mongo.Auth)
	...
	logger.Info("MongoDB bağlantısı başarılı, listDatabases komutu çalıştırılıyor")
	
	// listDatabases komutunu çalıştır
	result, err := c.runAdminCommand(client, bson.D{{Key: "listDatabases", Value: 1}})
	if err != nil {
		logger.Error("listDatabases komutu çalıştırılamadı: %v", err)
		return nil, fmt.Errorf("listDatabases komutu çalıştırılamadı: %v", err)
	}

	logger.Info("listDatabases komutu başarılı")

	// Databases array'ını çıkar
	databases, ok := result["databases"].(bson.A)
	if !ok {
		logger.Error("databases dizisi bulunamadı")
		return nil, fmt.Errorf("databases dizisi bulunamadı")
	}

	logger.Info("MongoDB'den %d database bulundu", len(databases))

	var dbList []map[string]interface{}
	for i, db := range databases {
		logger.Debug("Database %d işleniyor", i)
		
		// Hem bson.D hem de bson.M/map tiplerini destekle
		var dbMap map[string]interface{}
		
		if dbDoc, ok := db.(bson.D); ok {
			// bson.D tipinde
			dbMap = dbDoc.Map()
			logger.Debug("Database %d bson.D tipinde", i)
		} else if dbMapDirect, ok := db.(bson.M); ok {
			// bson.M tipinde
			dbMap = dbMapDirect
			logger.Debug("Database %d bson.M tipinde", i)
		} else if dbMapInterface, ok := db.(map[string]interface{}); ok {
			// map[string]interface{} tipinde
			dbMap = dbMapInterface
			logger.Debug("Database %d map[string]interface{} tipinde", i)
		} else {
			logger.Error("Database %d desteklenmeyen tipte: %T", i, db)
			continue
		}
		
		// Database bilgilerini map'e dönüştür
		dbInfo := make(map[string]interface{})
		
		if name, exists := dbMap["name"]; exists {
			if nameStr, ok := name.(string); ok {
				logger.Debug("Database bulundu: %s", nameStr)
				
				// System veritabanlarını filtrele
				if nameStr == "admin" || nameStr == "local" || nameStr == "config" {
					logger.Debug("System database filtrelendi: %s", nameStr)
					continue
				}
				dbInfo["name"] = nameStr
				logger.Debug("Database eklendi: %s", nameStr)
			}
		}
		
		if sizeOnDisk, exists := dbMap["sizeOnDisk"]; exists {
			dbInfo["sizeOnDisk"] = sizeOnDisk
		}
		
		if empty, exists := dbMap["empty"]; exists {
			dbInfo["empty"] = empty
		}
	...
	logger.Info("Filtrelemeden sonra %d database kaldı", len(dbList))
	return dbList, nil
}

// GetIndexStats belirtilen database için index kullanım istatistiklerini getirir
func (c *MongoCollector) GetIndexStats(databaseName string) (map[string]interface{}, error) {
	logger.Info("GetIndexStats fonksiyonu çağrıldı - Database: %s", databaseName)
	...
	if err != nil {
		logger.Error("Collections listelenemedi: %v", err)
		return nil, fmt.Errorf("collections listelenemedi: %v", err)
	}

	var collectionsData []map[string]interface{}

	for _, collName := range collections {
		// System collection'ları atla
		...
		if err != nil {
			logger.Warning("Collection stats alınamadı %s: %v", collName, err)
			continue
		}

		collReport := map[string]interface{}{
			"collectionName": collName,
			"documentCount":  0,
			"storageSize":    0,
			"indexes":        []map[string]interface{}{},
			"estimatedTopQueryPattern": nil,
		}

		// Document count
		if count, ok := stats["count"]; ok {
			collReport["documentCount"] = count
		}

		// Storage size
		if size, ok := stats["size"]; ok {
			collReport["storageSize"] = size
		}

		// Index kullanım istatistiklerini al
		indexStatsPipeline := mongo.Pipeline{
			{{"$indexStats", bson.D{}}},
			{{"$sort", bson.D{{"accesses.ops", -1}}}},
		}

		indexCursor, err := coll.Aggregate(ctx, indexStatsPipeline)
		if err != nil {
			logger.Warning("Index stats alınamadı %s: %v", collName, err)
		} else {
			var indexUsage []bson.M
			if err := indexCursor.All(ctx, &indexUsage); err != nil {
				logger.Warning("Index usage decode edilemedi %s: %v", collName, err)
			} else {
				var indexesData []map[string]interface{}

				// Index listesini al
				indexCursor, err := coll.Indexes().List(ctx)
				if err != nil {
					logger.Warning("Index listesi alınamadı %s: %v", collName, err)
				} else {
					var allIndexes []bson.M
					if err := indexCursor.All(ctx, &allIndexes); err != nil {
						logger.Warning("Index listesi decode edilemedi %s: %v", collName, err)
					} else {
						// Index kullanım bilgilerini birleştir
						for _, idxUsage := range indexUsage {
							indexName, _ := idxUsage["name"].(string)
							ops := int64(0)
							if access, ok := idxUsage["accesses"].(bson.M); ok {
								if opsVal, ok := access["ops"]; ok {
									switch v := opsVal.(type) {
									case int32:
										ops = int64(v)
									case int64:
										ops = v
							...
							indexesData = append(indexesData, map[string]interface{}{
								"name":   indexName,
								"ops":    ops,
								"fields": fields,
							})
						...
						// En çok kullanılan index için tahmini query pattern
						if len(indexesData) > 0 && len(indexesData[0]["fields"].([]string)) > 0 {
							topQueryPattern := make(map[string]interface{})
							for _, field := range indexesData[0]["fields"].([]string) {
								topQueryPattern[field] = "<value>"
							}
							collReport["estimatedTopQueryPattern"] = topQueryPattern
						}
					}
				}

				collReport["indexes"] = indexesData
			}
		}

		collectionsData = append(collectionsData, collReport)
	}

	// Convert to JSON string to avoid protobuf serialization issues
	collectionsJSON, err := json.Marshal(collectionsData)
	if err != nil {
		return nil, fmt.Errorf("collections JSON serialization hatası: %v", err)
	}
	
	report["collections"] = string(collectionsJSON)
	logger.Info("Index stats başarıyla toplandı - %d collection işlendi", len(collectionsData))
