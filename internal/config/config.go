...
type AgentConfig struct {
	...
	// Logging Ayarları
	Logging struct {
		Level       string `yaml:"level"`        // DEBUG, INFO, WARNING, ERROR, FATAL
		UseEventLog bool   `yaml:"use_eventlog"` // Windows Event Log kullanımı
		UseFileLog  bool   `yaml:"use_filelog"`  // Dosya log kullanımı
	} `yaml:"logging"`
}
...
func createDefaultAgentConfig(configPath string) (*AgentConfig, error) {
	...
	// Logging varsayılan ayarları
	config.Logging.Level = "INFO"
	config.Logging.UseEventLog = true  // Windows'da Event Log kullan
	config.Logging.UseFileLog = true   // Her zaman dosya log kullan
