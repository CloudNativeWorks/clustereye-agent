...
func (m *AlarmMonitor) checkMSSQLDeadlocks() {
	...
	// Son 1 saat içindeki deadlock olaylarını sorgula - SQL Server 2016+ uyumlu
	query := `
	WITH RawDeadlocks AS (
		SELECT
			CAST(event_data AS XML) AS DeadlockGraph,
			-- SQL Server 2016 ve öncesinde 'timestamp' sütunu kullanılıyor
			CAST(timestamp AS datetime) AS LocalTimeStamp
		FROM sys.fn_xe_file_target_read_file('system_health*.xel', NULL, NULL, NULL)
		WHERE object_name = 'xml_deadlock_report'
		AND CAST(timestamp AS datetime) > DATEADD(HOUR, -1, GETDATE())
	),
	DeadlockInfo AS (
		SELECT
			LocalTimeStamp,
