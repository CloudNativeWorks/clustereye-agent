...
func (r *Reporter) ShouldSkipConnection() bool {
	...
	// DON'T skip connection due to unhealthy state - we need to reconnect to become healthy!
	// Commenting out the unhealthy check to allow reconnection attempts
	// if !r.IsHealthy() {
...
func (r *Reporter) connectWithBackoff() error {
	...
	r.isHealthy = false
	r.connectionInterval = 2 * time.Minute // Reduced from 2 minutes to 60 seconds
...
func (r *Reporter) SendMSSQLInfo() error {
	...
	// KRİTİK KONTROL: Çelişkili durumları tespit et ve düzelt
	// STANDALONE olarak işaretlenmiş ama cluster bilgisi olan node'ları tespit et
	if mssqlInfo.NodeStatus == "STANDALONE" && (mssqlInfo.ClusterName != "" || mssqlInfo.IsHAEnabled) {
		log.Printf("SENDER KRİTİK UYARI: Çelişkili durum tespit edildi - NodeStatus=STANDALONE ama ClusterName='%s' veya IsHAEnabled=%v",
			mssqlInfo.ClusterName, mssqlInfo.IsHAEnabled)
		log.Printf("Bu failover anında yakalanmış olabilir, veri düzeltiliyor ve gönderiliyor")

		// Çelişkili durumda UNKNOWN olarak düzelt
		mssqlInfo.NodeStatus = "UNKNOWN"
		mssqlInfo.HARole = "UNKNOWN"

		// Eğer ClusterName varsa, kesinlikle HA enabled olmalı
		if mssqlInfo.ClusterName != "" {
			mssqlInfo.IsHAEnabled = true
		}

		log.Printf("SENDER düzeltilmiş durumlar: NodeStatus=%s, HARole=%s, IsHAEnabled=%v",
			mssqlInfo.NodeStatus, mssqlInfo.HARole, mssqlInfo.IsHAEnabled)
	}

	// Eğer NodeStatus UNKNOWN ise ve ClusterName varsa, HA enabled olmalı
	if mssqlInfo.NodeStatus == "UNKNOWN" && mssqlInfo.ClusterName != "" {
		log.Printf("SENDER UYARI: NodeStatus=UNKNOWN ama ClusterName='%s' - HA enabled olarak işaretleniyor", mssqlInfo.ClusterName)
		mssqlInfo.IsHAEnabled = true
	}
	...
	for {
		...
		default:
			...
			if query := msg.GetQuery(); query != nil {
				...
				// MongoDB MONGO_CURRENTOP komutu için özel işleme
				if r.platform == "mongo" && strings.HasPrefix(query.Command, "MONGO_CURRENTOP") {
					log.Printf("MongoDB currentOp komutu tespit edildi: %s", query.Command)

					// Komuttan secsRunning parametresini çıkar
					// Format: MONGO_CURRENTOP|secsRunning
					secsRunning := 0.1 // default değer
					parts := strings.Split(query.Command, "|")
					if len(parts) > 1 {
					...
					log.Printf("MongoDB currentOp parametreleri: secsRunning=%.1f", secsRunning)
					...
					// currentOp komutunu çalıştır
					currentOpResult, err := mongoCollector.GetCurrentOp(secsRunning)
					if err != nil {
						log.Printf("MongoDB currentOp komutu başarısız: %v", err)
						queryResult := map[string]interface{}{
							"status":  "error",
							"message": fmt.Sprintf("MongoDB currentOp komutu başarısız: %v", err),
						}
					...
					// Veriyi temizle (primitive.Timestamp vs. için)
					sanitizedResult := sanitizeData(currentOpResult)
					...
					log.Printf("MongoDB currentOp komutu başarıyla tamamlandı, secsRunning=%.1f", secsRunning)
					sendQueryResult(r.stream, query.QueryId, queryResult)
					isProcessingQuery = false
					continue
				}
				...
				// MongoDB database listesi için özel işleme
				if strings.HasPrefix(query.Command, "list_mongo_databases") {
					log.Printf("MongoDB database listesi sorgusu tespit edildi")
					...
					databases, err := mongoCollector.ListMongoDatabases()
					if err != nil {
						log.Printf("MongoDB database listesi alınamadı: %v", err)
						queryResult := map[string]interface{}{
							"status":  "error",
							"message": fmt.Sprintf("MongoDB database listesi alınamadı: %v", err),
						}
					...
					log.Printf("MongoDB database listesi başarıyla alındı: %d database bulundu", len(databases))

					// Database array'ini protobuf-compatible formata çevir
					var databaseList []interface{}
					for _, db := range databases {
						// map[string]interface{} -> interface{} dönüşümü
						dbConverted := make(map[string]interface{})
						for k, v := range db {
							dbConverted[k] = v
						}
						databaseList = append(databaseList, dbConverted)
					}
					...
					log.Printf("MongoDB database response oluşturuluyor: %+v", queryResult)

					resultStruct, err := structpb.NewStruct(queryResult)
					if err != nil {
						log.Printf("Struct oluşturma hatası: %v", err)
					} else {
						log.Printf("Struct başarıyla oluşturuldu: %+v", resultStruct)
					}

					anyResult, err := anypb.New(resultStruct)
					if err != nil {
						log.Printf("Any oluşturma hatası: %v", err)
					} else {
						log.Printf("Any başarıyla oluşturuldu, TypeUrl: %s", anyResult.TypeUrl)
					}
					...
					if err := r.stream.Send(response); err != nil {
						...
						log.Printf("MongoDB database listesi başarıyla gönderildi")
					}
				...
				// MongoDB index stats için özel işleme
				if strings.HasPrefix(query.Command, "getIndexStats") {
					log.Printf("MongoDB index stats sorgusu tespit edildi")
					...
					if len(parts) > 1 {
						if err := json.Unmarshal([]byte(parts[1]), &params); err != nil {
							log.Printf("Index stats parametreleri parse edilemedi: %v", err)
							queryResult := map[string]interface{}{
								"status":  "error",
								"message": fmt.Sprintf("Parametreler parse edilemedi: %v", err),
							}
							sendQueryResult(r.stream, query.QueryId, queryResult)
							isProcessingQuery = false
							continue
					...
					// Database name kontrolü
					databaseName, ok := params["database"].(string)
					if !ok || databaseName == "" {
						log.Printf("Database name parametresi eksik")
						queryResult := map[string]interface{}{
							"status":  "error",
							"message": "Database name parametresi gerekli",
						}
						sendQueryResult(r.stream, query.QueryId, queryResult)
						isProcessingQuery = false
						continue
					...
					// Index stats'ları al
					indexStats, err := mongoCollector.GetIndexStats(databaseName)
					if err != nil {
						log.Printf("MongoDB index stats alınamadı: %v", err)
						queryResult := map[string]interface{}{
							"status":  "error",
							"message": fmt.Sprintf("Index stats alınamadı: %v", err),
						}
						sendQueryResult(r.stream, query.QueryId, queryResult)
						isProcessingQuery = false
						continue
					...
					log.Printf("MongoDB index stats response oluşturuluyor")
				...
				// Patroni komutları için özel işleme (PostgreSQL platform'unda)
				if r.platform == "postgres" && (strings.HasPrefix(query.Command, "PATRONI_CMD") ||
					strings.HasPrefix(query.Command, "PATRONI_STATUS") ||
					strings.HasPrefix(query.Command, "PATRONI_COMMANDS")) {

					log.Printf("Patroni komutu tespit edildi: %s", query.Command)
					result := r.processPatroniCommand(query.Command)

					// Sonucu structpb'ye dönüştür
					resultStruct, err := structpb.NewStruct(result)
					if err != nil {
						log.Printf("Patroni sonucu struct'a dönüştürme hatası: %v", err)
						isProcessingQuery = false
						continue
					}
					...
					anyResult, err := anypb.New(resultStruct)
					if err != nil {
						log.Printf("Patroni sonucu Any tipine dönüştürülemedi: %v", err)
						isProcessingQuery = false
						continue
					}
					...
					if err := r.stream.Send(response); err != nil {
						log.Printf("Patroni sorgu cevabı gönderilemedi: %v", err)
					} else {
						log.Printf("Patroni komutu başarıyla işlendi: %s", query.Command)
					}
...
func (r *Reporter) reconnect() error {
	...
	// Yeniden bağlantı için daha fazla deneme yapalım
...
func (r *Reporter) SendPostgresInfo() error {
	...
	freeDisk, fdPercent, _, _, _ := postgres.GetDiskUsage()
...
func (r *Reporter) PromotePostgresToMaster(ctx context.Context, req *pb.PostgresPromoteMasterRequest) (*pb.PostgresPromoteMasterResponse, error) {
	...
	freeDisk, diskPercent, _, _, _ := postgres.GetDiskUsage()
...
func (r *Reporter) collectMSSQLSystemMetrics() *pb.SystemMetrics {
	...
	if runtime.GOOS == "windows" {
		log.Printf("[METRICS DEBUG] Windows sistem metrikleri toplanıyor...")

		// CPU kullanımı
		log.Printf("[METRICS DEBUG] CPU kullanımı ölçülüyor...")
		cpuUsage := r.getWindowsCPUUsage()
		log.Printf("[METRICS DEBUG] CPU kullanımı sonucu: %.2f", cpuUsage)

		if cpuUsage >= 0 {
			metrics.CpuUsage = cpuUsage
			log.Printf("[METRICS DEBUG] ✓ CPU kullanımı metriğe eklendi: %.2f%%", cpuUsage)
		} else {
			log.Printf("[METRICS DEBUG] ✗ CPU kullanımı negatif, metriğe eklenmiyor")
		}
...
func (r *Reporter) getWindowsCPUUsage() float64 {
	log.Printf("[CPU DEBUG] Windows CPU kullanımı ölçümü başlıyor...")

	// 1. Yöntem: WMIC ile CPU kullanımını al
	log.Printf("[CPU DEBUG] 1. Yöntem: WMIC komutu deneniyor...")
	cmd := exec.Command("wmic", "cpu", "get", "loadpercentage", "/value")
	out, err := cmd.Output()
	if err == nil {
		outStr := string(out)
		log.Printf("[CPU DEBUG] WMIC çıktısı: %q", outStr)

		re := regexp.MustCompile(`LoadPercentage=(\d+)`)
		matches := re.FindStringSubmatch(outStr)
		log.Printf("[CPU DEBUG] Regex eşleşmeleri: %v", matches)

		if len(matches) > 1 {
			if usage, err := strconv.ParseFloat(matches[1], 64); err == nil {
				log.Printf("[CPU DEBUG] ✓ WMIC ile CPU kullanımı başarıyla alındı: %.2f%%", usage)
				return usage
			} else {
				log.Printf("[CPU DEBUG] ✗ WMIC parse hatası: %v", err)
			}
		} else {
			log.Printf("[CPU DEBUG] ✗ WMIC regex eşleşmedi")
		}
	} else {
		log.Printf("[CPU DEBUG] ✗ WMIC komutu hatası: %v", err)
	}

	// 2. Yöntem: WMIC başarısız olursa typeperf kullan (Windows 2016 için daha güvenilir)
	log.Printf("[CPU DEBUG] 2. Yöntem: TypePerf komutu deneniyor...")
	cmd = exec.Command("typeperf", "-sc", "1", "\\Processor(_Total)\\% Processor Time")
	out, err = cmd.Output()
	if err == nil {
		outStr := string(out)
		log.Printf("[CPU DEBUG] TypePerf çıktısı: %q", outStr)

		lines := strings.Split(outStr, "\n")
		log.Printf("[CPU DEBUG] TypePerf satır sayısı: %d", len(lines))

		// typeperf çıktısı CSV formatında, 3. satırda değer var
		if len(lines) >= 3 {
			log.Printf("[CPU DEBUG] TypePerf 3. satır: %q", lines[2])
			fields := strings.Split(lines[2], ",")
			log.Printf("[CPU DEBUG] TypePerf alanları: %v", fields)

			if len(fields) >= 2 {
				// Çift tırnaklı değeri temizle
				valueStr := strings.Trim(fields[1], "\"")
				log.Printf("[CPU DEBUG] TypePerf temizlenmiş değer: %q", valueStr)

				if cpuVal, err := strconv.ParseFloat(valueStr, 64); err == nil {
					log.Printf("[CPU DEBUG] ✓ TypePerf ile CPU kullanımı başarıyla alındı: %.2f%%", cpuVal)
					return cpuVal
				} else {
					log.Printf("[CPU DEBUG] ✗ TypePerf parse hatası: %v", err)
				}
			} else {
				log.Printf("[CPU DEBUG] ✗ TypePerf yeterli alan yok")
			}
		} else {
			log.Printf("[CPU DEBUG] ✗ TypePerf yeterli satır yok")
		}
	} else {
		log.Printf("[CPU DEBUG] ✗ TypePerf komutu hatası: %v", err)
	}

	// 3. Yöntem: PowerShell ile CPU kullanımını al (son çare)
	log.Printf("[CPU DEBUG] 3. Yöntem: PowerShell komutu deneniyor...")
	...
	if err == nil {
		...
		log.Printf("[CPU DEBUG] PowerShell çıktısı: %q", cpuStr)

		if cpuVal, err := strconv.ParseFloat(cpuStr, 64); err == nil {
			log.Printf("[CPU DEBUG] ✓ PowerShell ile CPU kullanımı başarıyla alındı: %.2f%%", cpuVal)
			return cpuVal
		} else {
			log.Printf("[CPU DEBUG] ✗ PowerShell parse hatası: %v", err)
		}
	} else {
		log.Printf("[CPU DEBUG] ✗ PowerShell komutu hatası: %v", err)
	}

	log.Printf("[CPU DEBUG] ✗ Tüm yöntemler başarısız, CPU kullanımı alınamadı")
	return -1
}
...
func sanitizeData(data interface{}) interface{} {
	...
	case primitive.Timestamp:
		// Convert primitive.Timestamp to Unix timestamp
		return v.T
	...
	case primitive.DateTime:
		// Convert primitive.DateTime to RFC3339 string
		return v.Time().Format(time.RFC3339)

	case primitive.Binary:
		// Convert primitive.Binary to base64 string
		return fmt.Sprintf("binary_data_%d_bytes", len(v.Data))

	case primitive.Decimal128:
		// Convert primitive.Decimal128 to string
		return v.String()

	case primitive.JavaScript:
		// Convert primitive.JavaScript to string
		return string(v)

	case primitive.Symbol:
		// Convert primitive.Symbol to string
		return string(v)

	case primitive.CodeWithScope:
		// Convert primitive.CodeWithScope to map
		return map[string]interface{}{
			"code":  string(v.Code),
			"scope": sanitizeData(v.Scope),
		}

	case primitive.Regex:
		// Convert primitive.Regex to string
		return fmt.Sprintf("/%s/%s", v.Pattern, v.Options)

	case primitive.A:
		// Convert primitive.A (BSON array) to []interface{}
	...
	case primitive.D:
		// Convert primitive.D (BSON document) to map[string]interface{}
		result := make(map[string]interface{})
		for _, elem := range v {
			result[elem.Key] = sanitizeData(elem.Value)
		}
		return result

	case primitive.M:
		// Convert primitive.M (BSON map) to map[string]interface{}
		result := make(map[string]interface{})
	...
	case primitive.E:
		// Convert primitive.E (BSON element) to map
		return map[string]interface{}{
			"key":   v.Key,
			"value": sanitizeData(v.Value),
		}
...
// ExecutePatroniCommand executes a patronictl command via UI request
func (r *Reporter) ExecutePatroniCommand(ctx context.Context, req *pb.PatroniCommandRequest) (*pb.PatroniCommandResponse, error) {
	log.Printf("Patroni command execution requested: JobID=%s, Command=%s, Args=%v",
		req.JobId, req.Command, req.Args)

	// Create Patroni manager
	patroniManager := postgres.NewPatroniManager(r.cfg)

	// Check if Patroni is available
	available, err := patroniManager.IsPatroniAvailable()
	if !available {
		errMsg := fmt.Sprintf("Patroni not available: %v", err)
	...
	// Get current Patroni info
	pgCollector := postgres.NewPostgresCollector(r.cfg)
	patroniInfo := pgCollector.DetectPatroni()
	patroniManager.SetPatroniInfo(patroniInfo)

	// Execute the command
	result, err := patroniManager.ExecutePatroniCommand(req.Command, req.Args)
	if err != nil {
		errMsg := fmt.Sprintf("Patroni command execution failed: %v", err)
		log.Printf("ERROR: %s", errMsg)
		return &pb.PatroniCommandResponse{
			JobId:        req.JobId,
			Status:       pb.JobStatus_JOB_STATUS_FAILED,
	...
	// Prepare response
	response := &pb.PatroniCommandResponse{
		JobId:      req.JobId,
	...
	if result.Success {
		response.Status = pb.JobStatus_JOB_STATUS_COMPLETED
		log.Printf("Patroni command successful: %s", req.Command)
	} else {
		response.Status = pb.JobStatus_JOB_STATUS_FAILED
		response.ErrorMessage = result.Error
		log.Printf("Patroni command failed: %s - %s", req.Command, result.Error)
	}

	return response, nil
}

// GetPatroniClusterStatus gets the current Patroni cluster status
func (r *Reporter) GetPatroniClusterStatus(ctx context.Context, req *pb.PatroniStatusRequest) (*pb.PatroniStatusResponse, error) {
	log.Printf("Patroni cluster status requested: AgentID=%s", req.AgentId)
	...
	if !available {
		return &pb.PatroniStatusResponse{
			Status:       "error",
			ErrorMessage: fmt.Sprintf("Patroni not available: %v", err),
		}, nil
	}
	...
	// Get cluster status
	result, err := patroniManager.GetClusterStatus()
	if err != nil {
		return &pb.PatroniStatusResponse{
			Status:       "error",
			ErrorMessage: fmt.Sprintf("Failed to get cluster status: %v", err),
		}, nil
	}

	return &pb.PatroniStatusResponse{
		Status:      "success",
		ClusterName: patroniInfo.ClusterName,
		NodeRole:    patroniInfo.Role,
		NodeState:   patroniInfo.State,
		Output:      result.Output,
		LastUpdated: result.ExecutedAt.Unix(),
	}, nil
}

// GetPatroniAvailableCommands returns available Patroni commands for UI
func (r *Reporter) GetPatroniAvailableCommands(ctx context.Context, req *pb.PatroniCommandsRequest) (*pb.PatroniCommandsResponse, error) {
	log.Printf("Patroni available commands requested: AgentID=%s", req.AgentId)
	...
	pbCommands := make([]*pb.PatroniCommand, 0, len(commands))
	for name, cmd := range commands {
		pbCommand := &pb.PatroniCommand{
			Name:         name,
			Command:      cmd.Command,
			Args:         cmd.Args,
...
// processPatroniCommand processes Patroni commands received via query interface
func (r *Reporter) processPatroniCommand(command string) map[string]interface{} {
	log.Printf("Processing Patroni command: %s", command)

	// Patroni komutları için özel işleme
	if strings.HasPrefix(command, "PATRONI_CMD") {
		return r.processPatroniCmdCommand(command)
	} else if strings.HasPrefix(command, "PATRONI_STATUS") {
		return r.processPatroniStatusCommand(command)
	} else if strings.HasPrefix(command, "PATRONI_COMMANDS") {
		return r.processPatroniCommandsCommand(command)
	} else if strings.HasPrefix(command, "PATRONI_TEST") {
		return r.processPatroniTestCommand(command)
	} else if strings.HasPrefix(command, "PATRONI_FAILOVER") {
		return r.processPatroniFailoverCommand(command)
	} else if strings.HasPrefix(command, "PATRONI_SWITCHOVER") {
		return r.processPatroniSwitchoverCommand(command)
	}

	return map[string]interface{}{
		"status":  "error",
		"message": "Unknown Patroni command: " + command,
	}
}

// processPatroniCmdCommand processes PATRONI_CMD|command|args format
func (r *Reporter) processPatroniCmdCommand(command string) map[string]interface{} {
	// Komut formatı: PATRONI_CMD|command|args
	parts := strings.Split(command, "|")
	if len(parts) < 2 {
		return map[string]interface{}{
			"status":  "error",
			"message": "Invalid Patroni command format. Expected: PATRONI_CMD|command|args",
		}
	}

	// Komut ve argümanları çıkar
	patroniCommand := strings.TrimSpace(parts[1])
	args := []string{}
	if len(parts) > 2 {
	...
	log.Printf("Executing Patroni command: %s, args: %v", patroniCommand, args)
	...
	if !available {
		return map[string]interface{}{
			"status":  "error",
			"message": fmt.Sprintf("Patroni not available: %v", err),
		}
	}
	...
	// Komutu çalıştır
	result, err := patroniManager.ExecutePatroniCommand(patroniCommand, args)
	if err != nil {
		return map[string]interface{}{
			"status":  "error",
			"message": fmt.Sprintf("Patroni command execution failed: %v", err),
		}
	}
...
// processPatroniStatusCommand processes PATRONI_STATUS command
func (r *Reporter) processPatroniStatusCommand(command string) map[string]interface{} {
	log.Printf("Processing Patroni status command")
	...
	available, err := patroniManager.IsPatroniAvailable()
	if !available {
		log.Printf("Patroni availability check failed: %v", err)
		return map[string]interface{}{
			"status":            "error",
			"error_message":     fmt.Sprintf("Patroni not available: %v", err),
			"patroni_available": false,
		}
	}

	log.Printf("Patroni is available, getting cluster status...")
	...
	if err != nil {
		...
		return map[string]interface{}{
			"status":        "error",
			"error_message": fmt.Sprintf("Failed to get cluster status: %v", err),
		}
	}

	log.Printf("Cluster status retrieved successfully: %s", clusterStatus.Output)
...
// processPatroniCommandsCommand processes PATRONI_COMMANDS command
func (r *Reporter) processPatroniCommandsCommand(command string) map[string]interface{} {
	log.Printf("Processing Patroni commands list request")
...
// processPatroniTestCommand processes PATRONI_TEST command for debugging
func (r *Reporter) processPatroniTestCommand(command string) map[string]interface{} {
	log.Printf("Processing Patroni test command")

	// Patroni manager oluştur
	patroniManager := postgres.NewPatroniManager(r.cfg)

	// Detaylı test sonuçlarını al
	testResults := patroniManager.TestPatroniSetup()

	// Patroni detection bilgilerini de ekle
	pgCollector := postgres.NewPostgresCollector(r.cfg)
	patroniInfo := pgCollector.DetectPatroni()
	...
	result := map[string]interface{}{
		"status":        "success",
		"patroni_setup": testResults,
		"patroni_detection": map[string]interface{}{
			"enabled":        patroniInfo.IsEnabled,
			"cluster_name":   patroniInfo.ClusterName,
			"role":           patroniInfo.Role,
			"state":          patroniInfo.State,
			"rest_api_port":  patroniInfo.RestAPIPort,
			"detection_info": patroniInfo.DetectionInfo,
		},
	}
...
// processPatroniFailoverCommand processes PATRONI_FAILOVER|target_node command
func (r *Reporter) processPatroniFailoverCommand(command string) map[string]interface{} {
	log.Printf("Processing Patroni failover command")

	// Komut formatı: PATRONI_FAILOVER|target_node (target_node opsiyonel)
	parts := strings.Split(command, "|")
	targetNode := ""
	if len(parts) > 1 {
	...
	// Patroni detection yap
	collector := postgres.NewPostgresCollector(r.cfg)
	patroniInfo := collector.DetectPatroni()
	patroniManager.SetPatroniInfo(patroniInfo)
	...
	if !available {
		return map[string]interface{}{
			...
			"command_type": "failover",
		}
	}

	// Failover işlemini gerçekleştir
	log.Printf("Performing failover to target node: %s", targetNode)
	result, err := patroniManager.PerformFailover(targetNode)
	if err != nil {
		return map[string]interface{}{
			"status":       "error",
...
// processPatroniSwitchoverCommand processes PATRONI_SWITCHOVER|target_node command
func (r *Reporter) processPatroniSwitchoverCommand(command string) map[string]interface{} {
	log.Printf("Processing Patroni switchover command")

	// Komut formatı: PATRONI_SWITCHOVER|target_node (target_node opsiyonel)
	parts := strings.Split(command, "|")
	targetNode := ""
	if len(parts) > 1 {
		targetNode = strings.TrimSpace(parts[1])
	...
	if !available {
		return map[string]interface{}{
			...
			"message":      fmt.Sprintf("Patroni not available: %v", err),
			"target_node":  targetNode,
			"command_type": "switchover",
		}
	}

	// Switchover işlemini gerçekleştir
	log.Printf("Performing switchover to target node: %s", targetNode)
	result, err := patroniManager.PerformSwitchover(targetNode)
	if err != nil {
		return map[string]interface{}{
			"status":       "error",
			"message":      fmt.Sprintf("Switchover failed: %v", err),
			"target_node":  targetNode,
			"command_type": "switchover",
		}
	}
