...
func main() {
	...
	// Configure enhanced logging from config file
	if cfg.Logging.Level != "" {
		configLevel := logger.ParseLevel(cfg.Logging.Level)
		loggingConfig := logger.LoggingConfig{
			UseEventLog: cfg.Logging.UseEventLog,
			UseFileLog:  cfg.Logging.UseFileLog,
			LogLevel:    configLevel,
		}
		
		// Override with command line flag if provided
		if *logLevelFlag != "WARNING" {
			loggingConfig.LogLevel = logger.ParseLevel(*logLevelFlag)
		}
		
		logger.ConfigureLogging(loggingConfig)
		logger.Info("Enhanced logging configured - EventLog: %v, FileLog: %v, Level: %s", 
			cfg.Logging.UseEventLog, cfg.Logging.UseFileLog, logger.LevelToString(loggingConfig.LogLevel))
	}
